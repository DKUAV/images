...
DKUAV / Luciole 项目的 CUDA/PyTorch 基础镜像。构建于 NVIDIA 官方 PyTorch 镜像之上，为所有 CUDA 家族的第二层镜像提供系统工具、pip 包及预配置的非 root 用户。
...
- 这是一个**第一层基础镜像**，已发布至 GHCR，供 [`luciole-humble-cuda-dev`](../luciole-humble-cuda-dev/README_zh.md) 和 [`luciole-humble-cuda-runtime`](../luciole-humble-cuda-runtime/README_zh.md) 以 `FROM ghcr.io/dkuav/luciole-cuda-base:latest` 方式使用。
- 由于 `pytorch:24.10-py3` 已内置 Python，构建时跳过 `02-python-install.sh`。
