...
- **ld.so.cache 注册 / HPC-X 符号修复**：NVIDIA NGC pytorch:24.10-py3（arm64）里的 HPC-X v2.20 同时附带了 UCX 1.17 + UCC 1.4——而 UCC 1.4 引用了只在 UCX 1.18+ 才有的符号 `ucs_config_doc_nop`。这让 arm64 base 上 `import torch` 因 `undefined symbol: ucs_config_doc_nop` 失败（torch 本身不用 libucc，但不慎被 `hpcx.conf` 暴露后落入坏链）。Dockerfile 末尾会从 `hpcx.conf` 中去掉 `/opt/hpcx/{ucc,ucx}/lib` 两行，把其他厂商 `.so` 树写入 `/etc/ld.so.conf.d/zz-ngc-extra.conf`，再执行 `ldconfig`。完整诊断见 [`docs/ld-cache-notes_zh.md`](../../docs/ld-cache-notes_zh.md)。
